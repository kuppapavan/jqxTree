...
export class BarGraphComponent implements OnInit,DoCheck {
  ...
  @Input() public barGraphData : BarGraph;
 @Input() barChartData: BarGraph;
...
handleBarChartClick(event) {
  ...
  const type = 'Monthly';
  const selectedDate = (this.barGraphData.dataProvider[event.item.index].Date);
  let json = { };
